...
-- 7. TASK USER MAPPING (With project_id FK)
...
CREATE TABLE IF NOT EXISTS project_folders (
    ...
    CONSTRAINT fk_project_folder_parent
        FOREIGN KEY (parent_folder_id) REFERENCES project_folders (folder_id) ON DELETE CASCADE
);

CREATE UNIQUE INDEX IF NOT EXISTS uq_project_folder_name
    ON project_folders (project_id, COALESCE(parent_folder_id, ''), folder_name);
...
CREATE INDEX IF NOT EXISTS idx_otp_challenges_user_active 
    ON otp_challenges (user_id, created_at DESC) WHERE consumed_at IS NULL;
